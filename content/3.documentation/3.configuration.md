---
title: "Configuração do Docus"
description: "Entenda como configurações o Docus."
---

# Configuração do Docus

Aprenda como configurar o Docus.

::code-group

```ts [app.config.ts minimalista]
...
Para atualizar a logo no cabeçalho, crie um componente em `components/Logo.vue` com sua própria logo.

A atual usanda na documentação esta na pasta `/public/img`.
...
```
::alert{type="warning"}
**Não há na documentação do WhyApp a documentação completa do Docus**. Para acessar a documentação completa do Docus, clique no botão abaixo.
::

:button-link[Documentação completa do Docus]{href="https://docus.dev/" icon="IconDocus" blank}
